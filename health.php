...
try {
    if (function_exists('getDB')) {
        $__allPlans = getDB()->query('SELECT id, title, badge, desc_text, premium_from, company, company_color, company_logo FROM products WHERE category = "health" AND is_active = 1 ORDER BY sort_order, id')->fetchAll();
    }
} catch (Throwable $e) { /* ignore */ }
...
<section class="pb-12">
    <div class="max-w-[1400px] mx-auto px-4 md:px-8">
        <h2 class="text-2xl font-bold text-brand-navy mb-2">ประกันสุขภาพทั้งหมด (<?= count($__allPlans) ?>)</h2>
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-5">
            ...
            <div class="group bg-white rounded-2xl border border-gray-100 hover:shadow-lg hover:border-brand-navy/20 transition p-6 flex flex-col">
                ...
                <div class="flex gap-2 mt-auto pt-3 border-t border-gray-100">
                    ...
                    <a href="/form/?plan=<?= urlencode('ประกันสุขภาพ') ?>&plan_name=<?= urlencode($__ap['title']) ?>" class="flex-1 text-center text-sm font-bold text-white bg-brand-green hover:bg-brand-greenHover rounded-lg py-2 transition">เลือกแผนนี้</a>
                </div>
            </div>
            <?php endforeach; ?>
        </div>
